...
void MergeMatchingPoseParams( uintptr_t bonemerge, float* poses, float* target_poses ) {
   ...
   if ( *( DWORD* ) ( bonemerge + 16 ) && *( DWORD* ) ( bonemerge + 140 ) ) {
	  auto index = ( int* ) ( bonemerge + 32 );
	  int  v4 = 0;
	  do {
		 if ( *index != -1 ) {
			auto target = *( C_CSPlayer * * ) ( bonemerge + 4 );
...
namespace Engine
{
   ...
   void C_SetupBones::Setup( ) {
	  ...
	  CIKContext* m_pIk = *( CIKContext * * ) ( uintptr_t( this->m_animating ) + Engine::Displacement.C_BaseAnimating.m_pIk );
	  if ( !m_bShouldDoIK ) {
		 m_pIk = nullptr;
	  }
	  ...
	  if ( m_pIk ) {
		 ...
		 //Memory::VCall< oUpdateIKLocks >( this->m_animating, 191 )( this->m_animating, m_flCurtime );

		 m_pIk->UpdateTargets( this->m_vecBones, this->m_quatBones, this->m_boneMatrix, ( uint8_t* ) boneComputed );

		 typedef void( __thiscall * oCalculateIKLocks )( void*, float );
		 Memory::VCall< oCalculateIKLocks >( this->m_animating, 192 )( this->m_animating, m_flCurtime );
   ...
   bool CanBeAnimated( C_CSPlayer* player ) {
	  if ( !*( bool* ) ( uintptr_t( player ) + Engine::Displacement.DT_CSPlayer.m_bCustomPlayer ) || !player->m_PlayerAnimState( ) )
		 return false;
   ...
   void C_SetupBones::GetSkeleton( ) {
	  ...
	  static auto add_dependencies = Memory::Scan( XorStr( "client.dll" ), XorStr( "55 8B EC 81 EC BC 00 00 00 53 56 57" ) );
	  ...
	  boneSetup->AccumulatePose( this->m_vecBones, this->m_quatBones, *( int* ) ( uintptr_t( this->m_animating ) + 0x28BC ), *( float* ) ( uintptr_t( this->m_animating ) + 0xA14 ),
			   ...
			   for ( int i = 0; i < this->m_nAnimOverlayCount; ++i ) {
				  ...
				  if ( animLayer && animLayer->m_nSequence > 1 && animLayer->m_flWeight > 0.0f ) {
					 if ( this->m_bShouldDispatch && this->m_animating == C_CSPlayer::GetLocalPlayer( ) ) {
						...
						Memory::VCall< UpdateDispatchLayer >( this->m_animating, 246 )( this->m_animating, animLayer, world_hdr, animLayer->m_nSequence );
					 }
					 ...
					 if ( !this->m_bShouldDispatch || animLayer->m_nDispatchSequence_2 <= 0 || animLayer->m_nDispatchSequence_2 >= ( *( studiohdr_t * * ) world_hdr )->numlocalseq ) {
						...
						typedef void( __thiscall* oIKAddDependencies )( CIKContext*, float, int, int, float, float );

						BoneMergeCopyFromFollow( bone_merge, this->m_vecBones, this->m_quatBones, 0x40000, pos2, rot2 );
						if ( m_pIk )
						   ( oIKAddDependencies( add_dependencies ) ( m_pIk, *( float* ) ( uintptr_t( this->m_animating ) + 0xA14 ), animLayer->m_nSequence, animLayer->m_flCycle, boneSetup->m_flPoseParameter[ 2 ], animLayer->m_flWeight ) );
