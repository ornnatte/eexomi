...
void SimulationContext::InitSimulationContext( C_CSPlayer* player ) {
   static auto sv_maxspeed = Source::m_pCvar->FindVar( XorStr( "sv_maxspeed" ) );
   static auto sv_stepsize = Source::m_pCvar->FindVar( XorStr( "sv_stepsize" ) );
   ...
   this->gravity = g_Vars.sv_gravity->GetFloat( ) * Source::m_pGlobalVars->interval_per_tick * 0.5f;
...
#define MAX_CLIP_PLANES 5 
   ...
   allFraction = 0;
   time_left = Source::m_pGlobalVars->interval_per_tick;   // Total time for this movement operation.
...
#define CS_PLAYER_SPEED_DUCK_MODIFIER 0.34f
   ...
   static auto sv_enablebunnyhopping = Source::m_pCvar->FindVar( XorStr( "sv_enablebunnyhopping" ) );
   static auto sv_walkable_normal = Source::m_pCvar->FindVar( XorStr( "sv_enablebunnyhopping" ) );
   ...
   if ( this->flags & FL_ONGROUND ) {
	  ...
	  // Friction
	  {
		 static auto sv_stopspeed = Source::m_pCvar->FindVar( XorStr( "sv_stopspeed" ) );
		 static auto sv_friction = Source::m_pCvar->FindVar( XorStr( "sv_friction" ) );
		 ...
		 if ( speed <= 0.1f ) {
			...
			auto drop = ( control * friction ) * Source::m_pGlobalVars->interval_per_tick;
	  ...
	  // WalkMove
	  {
		 ...
		 this->m_vecVelocity.z = 0.0f;
		 {
			static auto sv_accelerate_use_weapon_speed = Source::m_pCvar->FindVar( XorStr( "sv_accelerate_use_weapon_speed" ) );
			...
			if ( addspeed > 0.0f ) {
			   ...
			   float v27 = ( ( Source::m_pGlobalVars->interval_per_tick * accel ) * wihspeedClamped );
			   ...
			   float v31 = fminf( ( ( Source::m_pGlobalVars->interval_per_tick * v30 ) * wihspeedClamped ), addspeed );
			   addspeed = v31;
		 ...
		 Vector dest;
		 dest[ 0 ] = this->m_vecOrigin[ 0 ] + this->m_vecVelocity[ 0 ] * Source::m_pGlobalVars->interval_per_tick;
		 dest[ 1 ] = this->m_vecOrigin[ 1 ] + this->m_vecVelocity[ 1 ] * Source::m_pGlobalVars->interval_per_tick;
	  ...
	  static auto sv_air_max_wishspeed = Source::m_pCvar->FindVar( XorStr( "sv_air_max_wishspeed" ) );
	  ...
	  if ( addspeed > 0.0 ) {
		 ...
		 auto v170 = std::fminf( addspeed, v169 * Source::m_pGlobalVars->interval_per_tick );
