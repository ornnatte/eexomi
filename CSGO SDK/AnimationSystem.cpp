...
namespace Engine
{
   ...
   void C_AnimationData::Update( ) {
	  ...
	  if ( should_calculate ) {
		 ...
		 if ( record->m_iChokeTicks == 1 ) {
			...
			//SimulateAnimations( record, previous_record, 1, max_desync_angle * 0.66f );
			//setup_side( lowright, &record->fakeLayersRightLow[ 0 ], &record->m_flAbsRotationRightLow, 3 );

			//SimulateAnimations( record, previous_record, -1, max_desync_angle * 0.66f );
			//setup_side( lowleft, &record->fakeLayersRightLow[ 0 ], &record->m_flAbsRotationLeftLow, 4 );
		 } else {
			SimulateAnimations( record, previous_record, 1, max_desync_angle );
			...
			//SimulateAnimations( record, previous_record, 1, max_desync_angle * 0.66f );
			//if ( delta < 0.0f )
			//   setup_side( lowright, &record->fakeLayersRightLow[ 0 ], &record->m_flAbsRotationRightLow, 4 );
			//else
			//   setup_side( lowleft, &record->fakeLayersLeftLow[ 0 ], &record->m_flAbsRotationLeftLow, 3 );
			...
			//   setup_side( lowleft, &record->fakeLayersLeftLow[ 0 ], &record->m_flAbsRotationLeftLow, 3 );
			//else
			//   setup_side( lowright, &record->fakeLayersRightLow[ 0 ], &record->m_flAbsRotationRight, 4 );
		 }
	  ...
	  if ( this->m_vecSimulationData.size( ) > 1 && current->m_iChokeTicks != 1 ) {
		 ...
		 if ( resolverIndex ) {
			...
			if ( previous.IsValid( ) && !previous->m_bIsInvalid ) {
			   ...
			   if ( resolverIndex <= 0 )
				  yaw = current->m_angEyeAngles.yaw - MAX_ROTATION;
			   else
				  yaw = current->m_angEyeAngles.yaw + MAX_ROTATION;
			}
			animState->m_flAbsRotation = std::remainderf( yaw, 360.0f );
   ...
   void C_AnimationData::AnimationResolver( Encrypted_t<Engine::C_AnimationRecord> current, Encrypted_t<Engine::C_AnimationRecord> previous ) {
	  ...
	  if ( current->m_fFlags & FL_ONGROUND && previous->m_fFlags & FL_ONGROUND && !current->m_bShiftingTickbase && !AnimationData->m_bSuppressAnimationResolver ) {
		 ...
		 if ( speed <= 0.1f ) {
		 ...
		 #if 0
			int test_side = 0;
			auto delta_fake = std::fabsf( current->m_serverAnimOverlays[ 6 ].m_flPlaybackRate - current->fakeLayersFake[ 6 ].m_flPlaybackRate );
			auto delta_right = std::fabsf( current->m_serverAnimOverlays[ 6 ].m_flPlaybackRate - current->fakeLayersRight[ 6 ].m_flPlaybackRate );
			auto delta_left = std::fabsf( current->m_serverAnimOverlays[ 6 ].m_flPlaybackRate - current->fakeLayersLeft[ 6 ].m_flPlaybackRate );

			if ( delta_fake < delta_right || delta_left < delta_right ) {
			   if ( delta_fake > delta_left && delta_right > delta_left ) {
				  test_side = -1;
			   }
			} else {
			   test_side = 1;
			...
			if ( test_side ) {
			   auto edict = GetServerEdict( player->m_entIndex );
			   if ( edict ) {
				  float  server_yaw = *( float* ) ( uintptr_t( edict ) + 0x1EC );
				  auto server_side = 2 * ( std::remainderf( server_yaw - current->m_angEyeAngles.yaw, 360.0f ) > 0.f ) - 1;
				  if ( server_side == test_side ) {
					 printf( "=======================================\n" );
					 printf( "server: lean: %d | yaw rate %d | yaw cycle %d\n", int( current->m_serverAnimOverlays[ 12 ].m_flWeight * 10000.0f ),
							 int( current->m_serverAnimOverlays[ 6 ].m_flWeight * 10000.0f ), int( current->m_serverAnimOverlays[ 6 ].m_flCycle * 10000.0f ) );
					 printf( "server previous: lean: %d | yaw rate %d | yaw cycle %d\n", int( previous->m_serverAnimOverlays[ 12 ].m_flWeight * 10000.0f ),
							 int( previous->m_serverAnimOverlays[ 6 ].m_flWeight * 10000.0f ), int( previous->m_serverAnimOverlays[ 6 ].m_flCycle * 10000.0f ) );
					 printf( "client: lean: %d | yaw rate %d | yaw cycle %d\n", int( current->fakeLayersFake[ 12 ].m_flWeight * 10000.0f ),
							 int( current->fakeLayersFake[ 6 ].m_flWeight * 10000.0f ), int( current->fakeLayersFake[ 6 ].m_flCycle * 10000.0f ) );
					 printf( "client previous: lean: %d | yaw rate %d | yaw cycle %d\n", int( previous->fakeLayersFake[ 12 ].m_flWeight * 10000.0f ),
							 int( previous->fakeLayersFake[ 6 ].m_flWeight * 10000.0f ), int( previous->fakeLayersFake[ 6 ].m_flCycle * 10000.0f ) );
					 printf( "=======================================\n" );
				  }
			   }
			}
	  ...
	  //if ( current->m_bAnimationResolverUsed ) {
		 //g_Vars.globals.m_iResolverType[ player->entindex( ) ] = 2;
	  //}
   }
}