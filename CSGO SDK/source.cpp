...
void __fastcall hkFireEvents( void* ecx, void* edx ) {
   ...
   if ( local && !local->IsDead( ) ) {
	  for ( CEventInfo* i = *( CEventInfo** ) ( uintptr_t( Source::m_pClientState.Xor( ) ) + 0x4E64 );
			i != nullptr;
			i = i->next ) {
...
namespace Source
{
   ...
   bool Create( void* reserved ) {
	  ...
	  m_pInput = ( IInput* ) ( *reinterpret_cast< IInput** >( ( *( uintptr_t** ) m_pClient.Xor( ) )[ 16 ] + 0x1 ) );
	  ...
	  m_pGlobalVars = ( **reinterpret_cast< CGlobalVars*** >( ( *reinterpret_cast< uintptr_t** >( m_pClient.Xor( ) ) )[ 0 ] + 0x1F ) );
	  ...
	  for ( auto clientclass = Source::m_pClient->GetAllClasses( );
		 ...
		 // почему-то ClassID отсутствуют в меню, поэтому сравниваем по имени
		 if ( !strcmp( clientclass->m_pNetworkName, XorStr( "CCSPlayer" ) ) ) {
			...
			// перехватываем функцию создания энтити игрока, теперь мы можем поставить хуки сразу при спавне игрока
			clientclass->m_pCreateFn = Hooked::hkCreateCCSPlayer;
			break;
		 }
	  }

	  // если инжектим уже в игре, то надо поставить хуки уже на существующих энтити
	  ...
	  // я в рот ебал получение пинга через нетчанал и ваш ебанный гет латенси, у меня свои методы
	  ...
	  //oResetGetWaterContentsForPointCache = Hooked::HooksManager.CreateHook<decltype( oResetGetWaterContentsForPointCache ) >( &hkResetGetWaterContentsForPointCache, ( void* ) reset_contents_cache );
	  ...
	  auto modify_eye_pos = Memory::Scan( XorStr( "client.dll" ), XorStr( "55 8B EC 83 E4 F8 83 EC 5C 53 8B D9 56 57 83" ) );//Engine::Displacement.Data.m_ModifyEyePos;
