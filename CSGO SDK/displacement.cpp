...
namespace Engine
{
   ...
   void Create( ) {
	  ...
	  Displacement.C_BaseAnimating.m_CachedBoneData = *( int* ) ( Memory::Scan( image_client, "FF B7 ?? ?? ?? ?? 52" ) + 2 ) + 0x4;
	  Displacement.C_BaseAnimating.m_AnimOverlay = *( int* ) ( Memory::Scan( image_client, "8B 89 ?? ?? ?? ?? 8D 0C D1" ) + 2 );

	  auto BoneSnapshotsCall = Memory::Scan( image_client, "8D 8F ?? ?? ?? ?? 6A 01 C7 87" );
	  ...
	  auto CacheBoneData = Memory::Scan( image_client, "8D 87 ?? ?? ?? ?? 50 E8 ?? ?? ?? ?? 8B 06" );
	  ...
	  Displacement.C_BaseAnimating.m_pStudioHdr = *( int* ) ( Memory::Scan( image_client, "8B B7 ?? ?? ?? ?? 89 74 24 20" ) + 0x2 ) + 0x4;
	  Displacement.C_BaseAnimating.m_bShouldDraw = *( int* ) ( Memory::Scan( image_client, "FF 15 ?? ?? ?? ?? 80 BE ?? ?? ?? ?? ?? 0F 84 ?? ?? ?? ??" ) + 0x8 );
	  Displacement.C_BaseAnimating.m_pBoneMerge = *( int* ) ( Memory::Scan( image_client, "89 86 ?? ?? ?? ?? E8 ?? ?? ?? ?? FF 75 08" ) + 2 );
	  Displacement.C_BaseAnimating.m_pIk = *( int* ) ( Memory::Scan( image_client, "8B 8F ?? ?? ?? ?? 89 4C 24 1C" ) + 2 ) + 4;
	  ...
	  Displacement.C_BasePlayer.m_pCurrentCommand = *( int* ) ( Memory::Scan( image_client, "89 ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 85 FF 75" ) + 2 );
	  auto relative_call = Memory::Scan( image_client, "E8 ?? ?? ?? ?? 83 7D D8 00 7C 0F" );
	  ...
	  Displacement.DT_BasePlayer.m_ubOldEFNoInterpParity = *( int* ) ( Memory::Scan( image_client, "8A 87 ?? ?? ?? ?? 8D 5F F8" ) + 2 ) + 8;

	  Displacement.C_CSPlayer.m_PlayerAnimState = *( int* ) ( Memory::Scan( image_client, "8B 8E ?? ?? ?? ?? 85 C9 74 3E" ) + 2 );
	  Displacement.C_CSPlayer.m_flSpawnTime = *( int* ) ( Memory::Scan( image_client, "89 86 ?? ?? ?? ?? E8 ?? ?? ?? ?? 80 BE ?? ?? ?? ?? ??" ) + 2 );
	  ...
	  Displacement.DT_CSPlayer.m_bCustomPlayer = *( int* ) ( Memory::Scan( image_client, "80 BF ?? ?? ?? ?? ?? 0F 84 ?? ?? ?? ?? 83 BF ?? ?? ?? ?? ?? 74 7C" ) + 2 );
	  ...
	  Displacement.DT_BaseCombatWeapon.m_CustomMaterials = ( *( int* ) ( Memory::Scan( image_client, "83 BE ? ? ? ? ? 7F 67" ) + 0x2 ) ) - 12;
	  Displacement.DT_BaseCombatWeapon.m_bCustomMaterialInitialized = *( int* ) ( Memory::Scan( image_client, "C6 86 ? ? ? ? ? FF 50 04" ) + 0x2 );
	  ...
	  Displacement.CBoneMergeCache.m_nConstructor = Memory::Scan( image_client, "56 8B F1 0F 57 C0 C7 86 ?? ?? ?? ?? ?? ?? ?? ??" );
	  Displacement.CBoneMergeCache.m_nInit = rel32_fix( Memory::Scan( image_client, "E8 ?? ?? ?? ?? FF 75 08 8B 8E ?? ?? ?? ??" ) );
	  Displacement.CBoneMergeCache.m_nUpdateCache = rel32_fix( Memory::Scan( image_client, "E8 ?? ?? ?? ?? 83 7E 10 00 74 64" ) );
	  Displacement.CBoneMergeCache.m_CopyToFollow = rel32_fix( Memory::Scan( image_client, "E8 ?? ?? ?? ?? 8B 87 ?? ?? ?? ?? 8D 8C 24 ?? ?? ?? ?? 8B 7C 24 18" ) );
	  Displacement.CBoneMergeCache.m_CopyFromFollow = rel32_fix( Memory::Scan( image_client, "E8 ?? ?? ?? ?? F3 0F 10 45 ?? 8D 84 24 ?? ?? ?? ??" ) );

	  Displacement.CIKContext.m_nConstructor = Memory::Scan( image_client, "53 8B D9 F6 C3 03 74 0B FF 15 ?? ?? ?? ?? 84 C0 74 01 CC C7 83 ?? ?? ?? ?? ?? ?? ?? ?? 8B CB" );
	  Displacement.CIKContext.m_nDestructor = Memory::Scan( image_client, "56 8B F1 57 8D 8E ?? ?? ?? ?? E8 ?? ?? ?? ?? 8D 8E ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 BE ?? ?? ?? ?? ??" );
	  Displacement.CIKContext.m_nInit = Memory::Scan( image_client, "55 8B EC 83 EC 08 8B 45 08 56 57 8B F9 8D 8F" );
	  Displacement.CIKContext.m_nUpdateTargets = Memory::Scan( image_client, "55 8B EC 83 E4 F0 81 EC ?? ?? ?? ?? 33 D2" );
	  Displacement.CIKContext.m_nSolveDependencies = Memory::Scan( image_client, "55 8B EC 83 E4 F0 81 EC ?? ?? ?? ?? 8B 81" );

	  Displacement.CBoneSetup.InitPose = Memory::Scan( image_client, "55 8B EC 83 EC 10 53 8B D9 89 55 F8 56 57 89 5D F4 8B 0B 89 4D F0" );
	  Displacement.CBoneSetup.AccumulatePose = Memory::Scan( image_client, "55 8B EC 83 E4 F0 B8 ?? ?? ?? ?? E8 ?? ?? ?? ?? A1 ?? ?? ?? ??" );
	  Displacement.CBoneSetup.CalcAutoplaySequences = Memory::Scan( image_client, "55 8B EC 83 EC 10 53 56 57 8B 7D 10 8B D9 F3 0F 11 5D ??" );
	  Displacement.CBoneSetup.CalcBoneAdj = Memory::Scan( image_client, "55 8B EC 83 E4 F8 81 EC ?? ?? ?? ?? 8B C1 89 54 24 04 89 44 24 2C 56 57 8B 00" );

	  // jmp patterns is not very reliable
	  auto CL_Predict = Memory::Scan( image_engine, "75 30 8B 87 ?? ?? ?? ??" );
	  auto CL_Move = Memory::Scan( image_engine, "74 0F 80 BF ?? ?? ?? ?? ??" );
	  ...
	  Displacement.Data.m_uMoveHelper = **( std::uintptr_t * * )( Memory::Scan( image_client, "8B 0D ?? ?? ?? ?? 8B 46 08 68" ) + 2 );
	  Displacement.Data.m_uInput = *( std::uintptr_t* )( Memory::Scan( image_client, "B9 ?? ?? ?? ?? F3 0F 11 04 24 FF 50 10" ) + 1 );
	  Displacement.Data.m_uPredictionRandomSeed = *( std::uintptr_t* )( Memory::Scan( image_client, "8B 0D ?? ?? ?? ?? BA ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4" ) + 2 );
	  Displacement.Data.m_uPredictionPlayer = *( std::uintptr_t* )( Memory::Scan( image_client, "89 ?? ?? ?? ?? ?? F3 0F 10 48 20" ) + 2 );
	  Displacement.Data.m_uModelBoneCounter = *( std::uintptr_t* )( InvalidateBoneCache + 0xA );
	  Displacement.Data.m_uClientSideAnimationList = *( std::uintptr_t* )( Memory::Scan( image_client, "A1 ?? ?? ?? ?? F6 44 F0 04 01 74 0B" ) + 1 );
	  Displacement.Data.m_uGlowObjectManager = *( std::uintptr_t* )( Memory::Scan( image_client, "0F 11 05 ?? ?? ?? ?? 83 C8 01" ) + 3 );
	  Displacement.Data.m_uCamThink = ( std::uintptr_t )( Memory::Scan( image_client, "85 C0 75 30 38 86" ) );
	  Displacement.Data.m_uRenderBeams = ( std::uintptr_t )( Memory::Scan( image_client, "A1 ?? ?? ?? ?? FF 10 A1 ?? ?? ?? ?? B9" ) + 0x1 );
	  Displacement.Data.m_uSmokeCount = *( std::uintptr_t* )( Memory::Scan( image_client, "55 8B EC 83 EC 08 8B 15 ?? ?? ?? ?? 0F 57 C0" ) + 0x8 );
	  Displacement.Data.m_uCenterPrint = ( std::uintptr_t )( Memory::Scan( image_client, "8B 35 ? ? ? ? 8D 4C 24 20" ) + 0x2 );
	  Displacement.Data.m_uHostFrameTicks = ( Memory::Scan( image_engine, "03 05 ? ? ? ? 83 CF 10" ) + 2 );
	  Displacement.Data.m_uServerGlobals = Memory::Scan( image_server, "8B 15 ? ? ? ? 33 C9 83 7A 18 01" ) + 0x2;
	  Displacement.Data.m_uServerPoseParameters = Memory::Scan( image_server, "8D 87 ? ? ? ? 89 46 08 C7 46 ? ? ? ? ? EB 06" ) + 0x2;
	  Displacement.Data.m_uServerAnimState = Memory::Scan( image_server, "8B 8F ?? ?? ?? ?? 85 C9 74 06 56" ) + 2;
	  Displacement.Data.m_uTicksAllowed = Memory::Scan( image_server, "FF 86 ?? ?? ?? ?? 8B CE 89 86 ?? ?? ?? ??" ) + 2;
	  Displacement.Data.m_uHudElement = Memory::Scan( image_client, "B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 5D 08" ) + 1;
	  Displacement.Data.m_uListLeavesInBoxReturn = Memory::Scan( image_client, "56 52 FF 50 18" ) + 0x5;
	  Displacement.Data.s_bAllowExtrapolation = Memory::Scan( image_client, "A2 ? ? ? ? 8B 45 E8" ) + 1;
	  Displacement.Data.m_FireBulletsReturn = Memory::Scan( image_client, "3B 3D ?? ?? ?? ?? 75 4C" );
	  Displacement.Data.m_D3DDevice = Memory::Scan( image_shaderapidx9, "A1 ?? ?? ?? ?? 50 8B 08 FF 51 0C" ) + 1;
	  Displacement.Data.m_SoundService = Memory::Scan( image_engine, "B9 ? ? ? ? 80 65 FC FE 6A 00" );
	  Displacement.Data.m_InterpolateServerEntities = Memory::Scan( image_client, "55 8B EC 83 EC 1C 8B 0D ? ? ? ? 53 56" ); // xref CVProfile::EnterScope(g_VProfCurrentProfile, "C_BaseEntity::InterpolateServerEntities", 0, "Interpolation", 0, 4);
	  Displacement.Data.m_SendNetMsg = Memory::Scan( image_engine, "55 8B EC 83 EC 08 56 8B F1 8B 86 ?? ?? ?? ?? 85 C0" ); // xref volume || ConVarRef %s doesn't point to an existing ConVar\n
	  Displacement.Data.m_ModifyEyePos = Memory::Scan( image_client, "55 8B EC 83 E4 F8 83 EC 5C 53 8B D9 56 57 83" ); // xref head_0
	  Displacement.Data.m_ResetContentsCache = Memory::Scan( image_client, "56 8D 51 3C BE" );
	  Displacement.Data.m_ProcessInterpolatedList = Memory::Scan( image_client, "0F B7 05 ? ? ? ? 3D ? ? ? ? 74 3F" ); // xref C_BaseEntity::InterpolateServerEntities
	  Displacement.Data.CheckReceivingListReturn = *reinterpret_cast< DWORD32* >( Memory::Scan( image_engine, "FF 50 34 8B 1D ? ? ? ? 85 C0 74 16 FF B6" ) + 0x3 );
	  Displacement.Data.ReadSubChannelDataReturn = *reinterpret_cast< DWORD32* >( Memory::Scan( image_engine, "FF 50 34 85 C0 74 12 53 FF 75 0C 68" ) + 0x3 );
	  Displacement.Data.SendDatagram = Memory::Scan( image_engine, "55 8B EC 83 E4 F0 B8 ? ? ? ? E8 ? ? ? ? 56 57 8B F9 89 7C 24 18" );
	  Displacement.Data.ProcessPacket = Memory::Scan( image_engine, "55 8B EC 83 E4 C0 81 EC ? ? ? ? 53 56 57 8B 7D 08 8B D9" );
	  Displacement.Data.m_GameRules = ( Memory::Scan( image_client, "8B 0D ?? ?? ?? ?? 85 C0 74 0A 8B 01 FF 50 78 83 C0 54" ) + 2 );
	  ...
	  Displacement.Function.m_uSetAbsOrigin = Memory::Scan( image_client, "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8" );
	  Displacement.Function.m_uSetAbsAngles = Memory::Scan( image_client, "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8" );
	  Displacement.Function.m_uIsBreakable = Memory::Scan( image_client, "55 8B EC 51 56 8B F1 85 F6 74 68" ); //xref
	  Displacement.Function.m_uClearHudWeaponIcon = Memory::Scan( image_client, "55 8B EC 51 53 56 8B 75 08 8B D9 57 6B FE 2C 89 5D FC" );
	  Displacement.Function.m_uLoadNamedSkys = Memory::Scan( image_engine, "55 8B EC 81 EC ?? ?? ?? ?? 56 57 8B F9 C7 45" );
	  Displacement.Function.m_uCreateAnimState = Memory::Scan( image_client, "55 8B EC 56 8B F1 B9 ?? ?? ?? ?? C7 46" );
	  Displacement.Function.m_uResetAnimState = Memory::Scan( image_client, "56 6A 01 68 ?? ?? ?? ?? 8B F1" );
	  Displacement.Function.m_uUpdateAnimState = Memory::Scan( image_client, "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24" );
	  Displacement.Function.m_uClanTagChange = Memory::Scan( image_engine, "53 56 57 8B DA 8B F9 FF 15" );
	  Displacement.Function.m_uGetSequenceActivity = Memory::Scan( image_client, "55 8B EC 53 8B 5D 08 56 8B F1 83" );
	  Displacement.Function.m_uInvalidatePhysics = Memory::Scan( image_client, "55 8B EC 83 E4 F8 83 EC 0C 53 8B 5D 08 8B C3 56 83 E0 04" );
	  Displacement.Function.m_uPostThinkVPhysics = Memory::Scan( image_client, "55 8B EC 83 E4 F8 81 ? ? ? ? ? 53 8B D9 56 57 83 ? ? ? ? ? ? 0F" );
	  Displacement.Function.m_SimulatePlayerSimulatedEntities = Memory::Scan( image_client, "56 8B F1 57 8B BE ?? ?? ?? ?? 83 EF 01 78 72 90 8B 86" );
	  Displacement.Function.m_uImplPhysicsRunThink = Memory::Scan( image_client, "55 8B EC 83 EC 10 53 56 57 8B F9 8B 87 ?? ?? ?? ?? C1 E8 16" );
	  Displacement.Function.m_uClearDeathNotices = Memory::Scan( image_client, "55 8B EC 83 EC 0C 53 56 8B 71 58" ); //55 8B EC 83 EC 0C 53 56 8B 71 58 33 DB 57 89 5D F8 8B 4E 04 8B 01 FF 90
	  Displacement.Function.m_uSetTimeout = Memory::Scan( image_engine, "55 8B EC 80 7D 0C 00 F3 0F 10 4D" );
	  Displacement.Function.m_uFindHudElement = Memory::Scan( image_client, "55 8B EC 53 8B 5D 08 56 57 8B F9 33 F6 39 77 28" );
	  Displacement.Function.m_SetCollisionBounds = Memory::Scan( image_client, "53 8B DC 83 EC 08 83 E4 F8 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 83 EC 10 56 57 8B 7B" );
	  Displacement.Function.m_MD5PseudoRandom = Memory::Scan( image_client, "55 8B EC 83 E4 F8 83 EC 70 6A 58 8D 44 24 1C 89 4C 24 08 6A 00 50" );
	  Displacement.Function.m_WriteUsercmd = Memory::Scan( image_client, "55 8B EC 83 E4 F8 51 53 56 8B D9 8B 0D" );
	  Displacement.Function.m_StdStringAssign = Memory::Scan( image_engine, "55 8B EC 53 8B 5D 08 56 8B F1 85 DB 74 57 8B 4E 14 83 F9 10 72 04 8B 06 EB 02" );
	  Displacement.Function.m_pPoseParameter = Memory::Scan( image_client, "55 8B EC 8B 45 08 57 8B F9 8B 4F 04 85 C9 75 15 8B" );
	  Displacement.Function.m_AttachmentHelper = Memory::Scan( image_client, "55 8B EC 83 EC 48 53 8B 5D 08 89 4D F4" );
	  Displacement.Function.m_LockStudioHdr = rel32_fix( Memory::Scan( image_client, "E8 ?? ?? ?? ?? 8D 47 FC" ) );
	  Displacement.Function.m_LineGoesThroughSmoke = Memory::Scan( image_client, "55 8B EC 83 EC 08 8B 15 ?? ?? ?? ?? 0F 57 C0" );
	  Displacement.Function.m_TraceFilterSimple = Memory::Scan( image_client, "55 8B EC 83 E4 F0 83 EC 7C 56 52" ) + 0x3D; //xref : offset ??_7CTraceFilterSimple@@6B@ ; const CTraceFilterSimple::`vftable' 
   }
